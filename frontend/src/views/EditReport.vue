<template>
  <div class="edit-report-page">
    <div class="page-container">
      ...
      <div v-if="reportStore.loading && !form.id" class="loading-container">
        <el-skeleton :rows="10" animated />
      </div>
      <div v-else-if="form.id" class="form-container card">
        <el-form
          ref="formRef"
          :model="form"
          :rules="rules"
...
import type { UpdateReportParams } from '@/types'
...
// 表单数据
const form = reactive<UpdateReportParams>({
  id: 0,
  device_name: '',
  fault_tags: [],
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        await reportStore.updateReport(form)
        ElMessage.success('报告更新成功')
        router.push(`/reports/${form.id}`)
      } catch (error) {
...
// 重置表单
const resetForm = () => {
  if (!formRef.value || !reportStore.currentReport) return
  
  // 重置为当前报告的数据
  Object.assign(form, {
    id: reportStore.currentReport.id,
    device_name: reportStore.currentReport.device_name,
    fault_tags: [...reportStore.currentReport.fault_tags],
    duration_minutes: reportStore.currentReport.duration_minutes,
    fault_category: reportStore.currentReport.fault_category,
    description: reportStore.currentReport.description,
    attachments: [...reportStore.currentReport.attachments]
  })
...
onMounted(async () => {
  try {
    await Promise.all([
      reportStore.fetchReportById(reportId.value),
      tagStore.fetchTags()
    ])
